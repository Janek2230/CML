...
constexpr int ROLE_TYP = Qt::UserRole + 1; // czy węzeł to Podejście czy Sesja
constexpr int ROLE_ID = Qt::UserRole + 2;  // ID rekordu w bazie
}
...
void SzczegolyWidget::ustawMedium(int idMedium) {
    ...
    for (const auto& medium : lista) {
        if (medium->getId() != idMedium) continue;
        ...
        if (medium->getRokWydania() > 0)
            podtytul += QString("  ·  %1").arg(medium->getRokWydania());
        if (!medium->getTworcy().trimmed().isEmpty())
            podtytul += QString("  ·  %1").arg(medium->getTworcy().trimmed());
        ...
        if (!czyZakonczone) {
            ...
            QString tooltipDaty = QString("Dodano: %1\nOstatnia aktywność: %2").arg(medium->getDataDodania().toString("dd.MM.yyyy"), medium->getDataOstatniejAktywnosci().toString("dd.MM.yyyy HH:mm"));
            QString tekstLicznikow = QString("W bibliotece od: <b>%1 dni</b>").arg(medium->getDataDodania().daysTo(teraz));
...
bool SzczegolyWidget::pokazDialogSesji(int& przyrost, int& sekundy, QString& notatka, const QString& tytul, bool edycja) {
    ...
    mainLayout->addWidget(groupPrzyrost);
    ...
    mainLayout->addWidget(groupNotatka);
    ...
    // blockSignals zapobiega nieskończonej pętli: zmiana spinnera wywołałaby manualChanged,
    // który nadpisałby totalSeconds i ponownie wywołał updateDisplay.
    auto updateDisplay = [&]() {
    ...
    // Ręczna zmiana któregokolwiek spinnera przelicza totalSeconds i odświeża wyświetlacz.
    auto manualChanged = [&]() {
        totalSeconds = spinH->value() * 3600 + spinM->value() * 60 + spinS->value();
        updateDisplay();
    };
    connect(spinH, QOverload<int>::of(&QSpinBox::valueChanged), this, manualChanged);
    connect(spinM, QOverload<int>::of(&QSpinBox::valueChanged), this, manualChanged);
    connect(spinS, QOverload<int>::of(&QSpinBox::valueChanged), this, manualChanged);
...
void SzczegolyWidget::onBtnCofnijZakonczenieClicked() {
                              ...
                              QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes)
    {
        ...
        if (appController.aktualizujPodejscie(idPodejscia, "W trakcie",
